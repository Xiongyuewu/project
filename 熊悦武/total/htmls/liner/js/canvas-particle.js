var CanvasParticle = (function(){
	...
	function canvasInit(canvasConfig){
		...
		var canvas = {
			...
			// 默认配置
			config: {
				vx: canvasConfig.vx || 20,
				vy:  canvasConfig.vy || 20,
				height: canvasConfig.height || 4,
				width: canvasConfig.width || 4,
				count: canvasConfig.count || 100,
				//color: canvasConfig.color || "121, 162, 185",
				//stroke: canvasConfig.stroke || "130,255,255",
				dist: canvasConfig.dist || 100,
				e_dist: canvasConfig.e_dist || 200000,
				max_conn: 1000
			}
		};
	...
	function drawPoint(canvas){
		...
		for(var i = 0, len = canvas.config.count; i < len; i++){
			...
			context.fillRect(point.x - canvas.config.width / 2, point.y - canvas.config.height, canvas.config.width * 4, canvas.config.height * 4);
			...
			for(var j = 0; j < len; j++){
				if(i != j){
					...
					if(dist <= canvas.config.dist && canvas.points[i].max_conn <canvas.config.max_conn){
						...
						context.strokeStyle = "rgb("+ randomColor()+ ","+ (1 - dist / canvas.config.dist) +")";
						context.beginPath();
